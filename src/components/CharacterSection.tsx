...
export function CharacterSection({ showId }: { showId: string }) {
  ...
  const openPicker = (charId: string) => {
  ...
  const handleCreateAndLink = async (charId: string) => {
    ...
    try {
      ...
      await handleLinkActor(charId, newActor.id);
    } catch (err) {
                  ...
                  <>
                    ...
                    {isAdmin && (
                      <div className="relative mt-1" ref={isPickerOpen ? pickerRef : undefined}>
                        <button
                          type="button"
                          onClick={() => isPickerOpen ? setLinkingCharId(null) : openPicker(char.id)}
                          ...
                          <div className={`absolute left-0 top-6 z-20 rounded-lg border border-ooo-slate bg-ooo-ink shadow-xl ${pickerMode === "create" ? "w-64" : "w-48 max-h-64 overflow-y-auto"}`}>
                            ...
                            {pickerMode === "choose" ? (
                              ...
                              <div className="p-3 space-y-2">
                                ...
                                <button
                                  type="button"
                                  onClick={() => handleCreateAndLink(char.id)}
                                  disabled={creating}
