...
const ALLOWED_ROLES = ["Creative", "admin", "Admin"];
...
export default function MyShowsPage() {
  ...
  const sessionLoading = sessionContext.loading;
  const userId = !sessionContext.loading && sessionContext.doesSessionExist ? sessionContext.userId : null;

  const isAllowed = !rolesLoading && roles.some((r) => ALLOWED_ROLES.includes(r));
  ...
  useEffect(() => {
    if (rolesLoading || sessionLoading) return;
    if (!isAllowed) {
      router.replace("/");
      return;
    }
    fetchItems();
  }, [isAllowed, rolesLoading, sessionLoading]);
  ...
  const fetchItems = async () => {
    try {
      ...
      const data: ShowItem[] = await res.json();
      const mine = data
  ...
  const handleDelete = async (id: string) => {
    ...
    try {
      ...
      if (!res.ok) throw new Error(res.status === 401 ? "Unauthorized" : "Failed to delete show");
      await fetchItems();
    } catch (err) {
      setError(err instanceof Error ? err.message : "An error occurred");
    }
  ...
  if (rolesLoading || sessionLoading || (loading && isAllowed)) {
    return (
      <div className="max-w-6xl mx-auto px-4 py-12">
        <div className="animate-pulse space-y-4">
          <div className="h-10 bg-ooo-slate/40 rounded w-1/3" />
  ...
  return (
    <div className="max-w-6xl mx-auto px-4 py-12">
      <div className="flex items-center justify-between mb-2">
        <h1 className="font-display text-4xl font-bold text-ooo-cream">My Shows</h1>
        <Link
          href="/shows/new?back=my-shows"
      ...
      <p className="text-ooo-muted mb-8">Shows you have registered and published.</p>
      ...
      {!loading && items.length === 0 && (
        <div className="bg-ooo-slate border border-ooo-ink rounded-lg p-12 text-center">
          <p className="text-ooo-muted">You haven&apos;t registered any shows yet.</p>
          <Link
            href="/shows/new?back=my-shows"
            className="mt-4 inline-block px-4 py-2 bg-ooo-accent text-ooo-black rounded-lg text-sm font-semibold hover:bg-ooo-accent/80 transition-colors"
          >
            Register your first show
          </Link>
        </div>
      )}
      ...
      {!loading && items.length > 0 && (
        <div className="grid gap-8 sm:grid-cols-2 lg:grid-cols-3">
          {items.map((item) => (
            <div
              key={item.id}
              className="relative bg-ooo-slate border border-ooo-ink rounded-lg overflow-hidden group hover:border-ooo-accent/50 transition-colors"
            >
              ...
              <div className="absolute top-2 right-2 flex gap-1">
                <Link
                  href={`/shows/${item.id}/edit?back=my-shows`}
